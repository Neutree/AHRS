	...
	:isBusySend(0),mUseDma(useDMA),mPrecision(3)
{
	...
	if(USART==1)
	{
		...
		if(remap==0x01)
		{
			...
			GPIO_InitStructureTx.GPIO_Pin = GPIO_Pin_6;
			GPIO_InitStructureRx.GPIO_Pin = GPIO_Pin_7;
			USART_GPIO=GPIOB;
	...
	else if(USART==3)
	{
		...
		else if(remap==0x11)
		{
			...
			GPIO_InitStructureTx.GPIO_Pin = GPIO_Pin_8;
			GPIO_InitStructureRx.GPIO_Pin = GPIO_Pin_9;
			USART_GPIO=GPIOD;
		}
		else
		{
	...
	GPIO_Init(USART_GPIO, &GPIO_InitStructureTx);    
	GPIO_Init(USART_GPIO, &GPIO_InitStructureRx);
