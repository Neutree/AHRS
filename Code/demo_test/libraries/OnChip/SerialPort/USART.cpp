...
bool USART::SendData(u8 txData[], u16 size)
{
	if(!mTxBuf.Puts(txData,size)) return false;      //add data to Tx buffer, if overflow, return false
...
{
	if(mRxBuf.Size()<number)//没有足够长的数据
		return false;
	else
	{
		mRxBuf.Gets(buffer,number);//数据出队
		return true;
	}
}
...
void USART::IRQ()
{
	...
	if(USART_GetITStatus(mUSARTx,USART_IT_RXNE)!=RESET)  //RxNE
	{
		...
		mRxBuf.Put(USART_ReceiveData(mUSARTx));            //receive byte
	}                                                    //TxE and TC
	if(USART_GetITStatus(mUSARTx,USART_IT_TXE)!=RESET || USART_GetITStatus(mUSARTx,USART_IT_TC)!=RESET)   
	{
